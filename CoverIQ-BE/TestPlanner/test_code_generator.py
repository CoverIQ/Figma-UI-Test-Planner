...
def generate_E2E_code(feature_text : List[str] , api_key: str )->Dict[str,Any]:
    result = {}
    result_count = 1
    for feature in feature_text :
        print(result_count)
        client = genai.Client(api_key=api_key)
        prompt =f''' Help me generate automated E2E testing code (Please do not include markdown formatting or triple backticks.Only return plain Python code.) with Selenium Webdriver for this Cucumber feature file. Cucumber feature file :{feature}'''
...
def generate_feature_text(test_cases: Dict[str, Any]) -> List[str]:
    ...
    for objective_key, objective in test_cases.items():
        ...
        for idx, description in enumerate(objective['bdd_style_descriptions'], 1):
            ...
            feature_texts.append(feature_content)
            feature_count += 1
